 ...
 * Endpoint AJAX: POST para crear nuevo plan gastronómico
...
if ($restaurante_id <= 0) {
    echo json_encode(['ok' => false, 'msg' => 'Debe seleccionar un restaurante.']);
    exit;
}
...
if (!$check_rest->fetch()) {
    echo json_encode(['ok' => false, 'msg' => 'Restaurante no válido.']);
    exit;
}
...
if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] === UPLOAD_ERR_OK) {
    ...
    $nombre_archivo = 'plan_gastronomico_' . time() . '_' . uniqid() . '.' . $ext;
